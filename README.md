# triage_engine —— 能源/零碳/AI 情报三库分类评分引擎（v1.09）
...
**v1.09**（2026-09-24）＝ v1.04 分类规则 ＋ v1.05 事件级去重 ＋ v1.06(b) 论文统一档位 ＋ v1.07 参考区 ＋ v1.08 未来态/汇总稿/媒体解读守卫 ＋ v1.09 展示链路与 LLM 预置叶冻结。
...
```bash
python -m triage_engine.cli version   # 验证：triage_engine v1.09 method=…display-chain-v109
```
...
└── tests/test_smoke.py                  # 合成数据冒烟测试（v1.07–v1.09 机制断言）
```
...
冒烟测试覆盖：v1.09 展示链路（论文/新闻两级＋归入大类 A/B/C/D，域外标域外，计数闭合）、
