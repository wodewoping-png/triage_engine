# triage_engine —— 能源/零碳/AI 情报三库分类评分引擎（v1.10）
...
**v1.10**（2026-09-24）＝ v1.04 分类规则 ＋ v1.05 事件级去重 ＋ v1.06(b) 论文统一档位 ＋ v1.07 参考区 ＋ v1.08 未来态/汇总稿/媒体解读守卫 ＋ v1.09 展示链路与 LLM 预置叶冻结 ＋ v1.10 载体前提（C-T16：DOI×语义共决，有 DOI ≠ 论文）。
...
```bash
python -m triage_engine.cli version   # 验证：triage_engine v1.10 method=…doi-carrier-premise-v110
```
...
│   ├── 00_严格语义分类与评分执行说明.md   # 执行总说明（v1.09；§9 = v1.05–v1.09 增量口径）
...
└── tests/test_smoke.py                  # 合成数据冒烟测试（v1.07–v1.10 机制断言）
```
...
冒烟测试覆盖：v1.10 载体前提（literature 一律 T01；wechat/news 的 DOI×题名媒体框架 → T23，全文转载载体 rescue → T01）、
